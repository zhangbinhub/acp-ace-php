...
class ToolsClass
{
    ...
    public static $LOGIN_YZM_STR = "portal_login_yzm";

    public static $LOGIN_USER_STR = "portal_loginUser";
        ...
        if (count($submenus) > 0) {
            ...
            foreach ($submenus as $submenu) {
                ...
                if (empty($submenu['page_url']) || count($submenus_tmp) > 0) {
                    ...
                    switch ($opentype) {
                        case 0:
                            $url = "javascript:portal_tools_obj.loadPageInDiv({menuid:'" . $id . "',title:'" . $submenu['name'] . "',path:'" . $this->buildOpentype($url, 0) . "'});";
                            break;
                        case 1:
                            $url = "javascript:portal_tools_obj.loadPageInDiv({menuid:'" . $id . "',title:'" . $submenu['name'] . "',path:'" . $this->buildOpentype($url, 1) . "'});";
                            break;
                        case 2:
    ...
    public function generateMenu($topmenus, $settings_hover = 0, $settings_highlight = 0)
    {
        ...
        foreach ($topmenus as $topmenu) {
            ...
            if (empty($topmenu['page_url']) || count($submenus_tmp) > 0) {
                ...
                switch ($opentype) {
                    case 0:
                        $url = "javascript:portal_tools_obj.loadPageInDiv({menuid:'" . $id . "',title:'" . $topmenu['name'] . "',path:'" . $this->buildOpentype($url, 0) . "'});";
                        break;
                    case 1:
                        $url = "javascript:portal_tools_obj.loadPageInDiv({menuid:'" . $id . "',title:'" . $topmenu['name'] . "',path:'" . $this->buildOpentype($url, 1) . "'});";
                        break;
                    case 2:
